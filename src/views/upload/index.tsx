...
export const UploadView: FC = ({}) => {
    ...
    const handleFileChange = async (event: any) => {
        ...
        //     Number(
        //         (
        //             await umi.uploader.getUploadPrice([_file])
    ...
    const upload = async () => {
        try {
            ...
            const pinata = new PinataSDK({
                ...
                pinataGatewayKey: process.env.NEXT_PUBLIC_PINATE_GATEWAY_KEY,
            ...
            let correctURI = upload.IpfsHash;
            console.log("uri: ", correctURI);
            setURI("https://gateway.pinata.cloud/ipfs/" + correctURI.toString());
            setUploading(false);
                    ...
                    <div>
                        <div className="flex justify-center  mb-10">
                            <form className="mt-[20%]">
                                <label
                                    htmlFor="file"
                                    className="font-bold py-2 px-2 bg-[#312d29] border border-[#c8ab6e] rounded-xl uppercase hover:cursor-pointer"
                                >
                                    Select file
                                    <input
                                        id="file"
                        ...
                        <div className="flex justify-center  mb-10">
                            <label>You selected file: {fileName}</label>
                        </div>

                        <div className="flex justify-center">
                            {!fileIsSelected && uploading == false && (
                                <button className="font-bold py-1 px-2 bg-[#312d29] border border-[#c8ab6e] rounded-xl hover:cursor-not-allowed	uppercase mb-4">
                                    Upload
                                </button>
                            )}
                            ...
                            {fileIsSelected && uploading == false && (
                                <button
                                    className="font-bold py-1 px-2 bg-[#312d29] border border-[#c8ab6e] rounded-xl uppercase mb-4"
                                    onClick={upload}
                                >
                                    Upload
                                </button>
                        ...
                        {/* <div className="flex justify-center">
                            {fileName != "" && uri == "" && uploadCost && (
                                <div className="text-white font-semibold text-lg mb-4">
                                    You will upload <strong>{fileName}</strong>{" "}
                                    for {uploadCost} SOL
                                </div>
                            )}
                        </div> */}
                        ...
                        <div className="flex justify-center">
                            {uri !== "" && (
                                <div className="font-semibold text-xl mb-[2%]">
                                    ✅ Successfuly uploaded! <br />
                                    Don&apos;t forget to copy the following
                                    link:
                                </div>
                            )}
                        </div>
